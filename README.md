...
- Removed the kid login screen's "My Therapist Set Me Up" name picker (2026-09-24) and retired its two unauthenticated backend routes, `GET /auth/kid-candidates` (which listed unlinked children's names to anyone, and never actually populated the picker) and `POST /auth/kid-pin-setup`; both now return 410. New kid accounts are created only by a parent or therapist, who sets the PIN
