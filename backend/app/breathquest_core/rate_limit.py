"""
app/breathquest_core/rate_limit.py — lightweight in-process rate limiting
and account lockout for the auth endpoints.

Ported from quest-games' core/rate_limit.py as part of the 2026-08-11
agenti_ai <-> quest-games merge. Logic is unchanged -- no model/app-specific
dependencies here, just an in-memory dict store.
...
2. Per-account failed-attempt lockout (`check_account_lockout` /
   ...
   against PIN/password brute-forcing.
