"""
Per-child tabular Q-agent persistence. Each child gets their own small
Q-table, seeded from a shared "prior" (trained on the simulator) so a
brand-new child doesn't start from nothing, then updated online from that
specific child's real transitions as they play.
"""
...
def load_child_agent(child_id, **agent_kwargs) -> TabularQAgent:
...
def save_child_agent(child_id, agent: TabularQAgent):
    child_path = Q_TABLES_DIR / f"{child_id}.json"
    save_q_table(agent.q_table, child_path)


def update_child_agent_from_transition(child_id, obs, action: int, reward: float,
