...
Ported from quest-games' services/weekly_summary.py. Deliberately NOT an
LLM call: no external API, fully deterministic given the same underlying
data, seeded from (patient_id, week_start) so re-generating for the same
patient/week always produces the same narrative.

Adapted for agenti_ai: BreathQuestPatient (not quest-games' standalone
Patient), app.retraining.data_store (Postgres, not SQLite -- returns real
datetime objects, not ISO strings), and VaakMirrorSession (agenti_ai's
actual class name; patient_id there is a loose String column, not a real
FK, so it's compared as str(patient.id)).
"""
...
from app.models.breathquest_models import (
    BreathQuestPatient, GameSession, Assignment, AssignmentStatus, Goal, HomePracticeLog,
)
from app.models.vaakmirror_models import VaakMirrorSession as VMSession, Attempt as VMAttempt, AttemptOutcome as VMOutcome
...
def _seeded_rng(patient_id, week_start: datetime) -> random.Random:
...
async def _week_bq_sessions(db: AsyncSession, patient_id, start: datetime, end: datetime):
...
async def _week_assignments(db: AsyncSession, patient_id, start: datetime, end: datetime):
    result = await db.execute(select(Assignment).where(Assignment.patient_id == patient_id))
...
async def _goals(db: AsyncSession, patient_id):
...
async def _week_practice_logs(db: AsyncSession, patient_id, start: datetime, end: datetime):
    result = await db.execute(
...
def _week_chime_events(patient_id, start: datetime, end: datetime, db_path):
    """app.retraining.data_store is Postgres-backed and returns real
    datetime objects for `timestamp` (not ISO strings like the old SQLite
    version) -- handled below without assuming either shape."""
...
async def _week_vaakmirror_attempts(db: AsyncSession, patient_id, start: datetime, end: datetime):
    """VaakMirrorSession.patient_id is a loose String column (not a real
    FK) -- compared as str(patient_id) to match whatever the VaakMirror
    auth layer wrote there."""
    result = await db.execute(
        select(VMAttempt, VMSession.game)
        .join(VMSession, VMAttempt.session_id == VMSession.id)
        .where(
            VMSession.patient_id == str(patient_id),
...
async def _week_voicehurdlerace_sessions(db: AsyncSession, patient_id, start: datetime, end: datetime):
    result = await db.execute(
...
async def generate_weekly_summary(
    ...
    if sessions:
        ...
        ctx = {
            "name": name, "n_sessions": n,
            "session_word": _plural(n, "session"), "times_word": _plural(n, "time"),
        ...
        if stars:
            ...
            star_ctx = {"avg_stars": avg_stars, "avg_stars_s": "" if avg_stars == 1 else "s", "prev_avg_stars": prev_avg}
    ...
    if chime_events:
        ...
        chime_ctx = {
            "name": name, "chime_n": n, "attempt_word": _plural(n, "attempt"),
    ...
    if vm_rows:
        ...
        vm_ctx = {
            "name": name, "vm_n": n, "attempt_word3": _plural(n, "attempt"),
            "vm_games": len(games_played), "game_word": _plural(len(games_played), "game"),
    ...
    if vhr_sessions:
        ...
        vhr_ctx = {
            "name": name, "vhr_n": n, "race_word": _plural(n, "race"),
            "vhr_stars": f"{avg_stars:.1f}", "vhr_pitch": f"{avg_pitch:.0f}",
        }
    ...
    if completed_assignments or overdue_assignments:
        ...
        assign_ctx = {
            "completed_assign": c, "completed_assign_s": "" if c == 1 else "s",
            "completed_assign_be": "was" if c == 1 else "were",
            "overdue_assign": o, "overdue_assign_be": "is" if o == 1 else "are",
        }
    ...
    if goals:
        ...
        if achieved_this_week:
            ...
            goal_ctx = {
                "n_achieved": n, "n_achieved_s": "" if n == 1 else "s",
                "n_achieved_be": "was" if n == 1 else "were",
                "goal_list": ", ".join(goal_names), "name": name,
            }
        ...
        if open_goals:
            ...
            open_ctx = {
                "n_open": n_open, "n_open_s": "" if n_open == 1 else "s",
                "n_open_verb": "s" if n_open == 1 else "", "n_open_be": "is" if n_open == 1 else "are",
    ...
    if practice_logs:
        ...
        practice_ctx = {
            "name": name, "practice_days": days, "day_word": _plural(days, "day"),
            "log_word": _plural(len(practice_logs), "log"), "practice_minutes": minutes,
        }
