...
class AgentDecisionOut(BaseModel):
    policy: str
    action: Literal["lower", "hold", "raise"]
    n_events_considered: int
    message: str
...
@router.get("/events/{child_id}", response_model=list[EventOut])
...
@router.get("/difficulty/{child_id}/{level_id}", response_model=DifficultyDecision)
...
# ============================================================
AGENT_MODELS_DIR = Path(__file__).resolve().parent.parent.parent / "agent" / "models"
...
@router.get("/agent/decide/{child_id}/{level_id}", response_model=AgentDecisionOut)
def agent_decide(
    child_id: str,
    level_id: str,
