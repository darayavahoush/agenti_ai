...
async def _accuracy_by(db: AsyncSession, patient_id: str, column) -> list[CategoryAccuracy]:
    stmt = (
        ...
        .where(VaakMirrorSession.patient_id == patient_id, column.isnot(None))
        .group_by(column)
    )
...
async def get_dashboard(
    ...
    sessions_count_result = await db.execute(
        select(func.count()).select_from(VaakMirrorSession).where(VaakMirrorSession.patient_id == patient_id)
    )
