...
assignments, goals, messages, home practice, alerts, weekly summaries,
sound progress, and PDF export.

History:
- Fixed 2026-08-11: was pointing at app.models.patient.Patient /
  app.models.session.Session (Assessment's models) instead of
  BreathQuestPatient/GameSession/TherapistNote. Also fixed two scoping bugs:
  get_dashboard_summary queried ALL patients with no therapist filter, and
  get_patient_progress had no ownership check (any therapist could view any
  other therapist's patient by ID). Notes now use the real TherapistNote
  model instead of 501/placeholder stubs.
- Merged 2026-08-12: layered quest-games' full feature set (assignments,
  goals, messages, home practice, alerts, weekly summaries, sound progress,
  ICF-style PDF export, multi-game dashboard summary across BreathQuest/
  VoiceHurdleRace/VaakMirror/Chime) onto the fix above. Every ownership
  check throughout uses _get_owned_patient against BreathQuestPatient
  (never Assessment's Patient). The old HTTP-based core.assessment_client /
  agent.diagnostic_client call quest-games made for latest_assessment is
  replaced by services/assessment_lookup.py's in-process query, now that
  Assessment's data lives in this same backend rather than a separate
  microservice.
"""
...
from app.models.breathquest_models import (
    BreathQuestPatient, GameSession, TherapistNote, LevelID,
...
RECENT_WINDOW = 10
_agent_service = AgentService(db_path=CHIME_DB_PATH, recent_window=RECENT_WINDOW)
...
async def _get_owned_patient(patient_id: str, therapist, db: AsyncSession) -> BreathQuestPatient:
    """Ownership check used throughout this router — 404 (not 403) on a
    patient that exists but isn't this therapist's, so we don't leak which
    patient_ids are real. Always BreathQuestPatient, never Assessment's
    Patient model."""
...
async def get_dashboard_summary(
    ...
    vhr_week = await db.execute(
        select(
            ...
            and_(
                VoiceHurdleRaceSession.patient_id.in_(patient_ids) if patient_ids else False,
    ...
    vm_week = await db.execute(
        select(func.count(VaakMirrorSession.id)).where(
            and_(
                VaakMirrorSession.patient_id.in_([str(pid) for pid in patient_ids]) if patient_ids else False,
    ...
    # chime_data_store.* is synchronous DB I/O — threaded off so it doesn't
    # block this async route's event loop (same fix applied throughout this
    # router; count_events in particular runs once per patient in the loop
    # below, so its cost compounds with clinic size if left blocking).
    chime_week_count = await asyncio.to_thread(
        chime_data_store.count_events_since,
    ...
    for p in patients:
        ...
        vm_count_result = await db.execute(
            ...
            .where(VaakMirrorSession.patient_id == str(p.id))
        )
...
async def get_patient_progress(
    ...
    for level_id, level_name in LEVEL_NAMES.items():
        level_sessions_completed = [s for s in completed if s.level_id == level_id]
        level_sessions_all = [s for s in sessions if s.level_id == level_id]
        if level_sessions_completed:
            best_stars = max(s.stars_earned or 0 for s in level_sessions_completed)
            avg_stars = sum(s.stars_earned or 0 for s in level_sessions_completed) / len(level_sessions_completed)
            breath_vals = [s.avg_breath_strength for s in level_sessions_completed if s.avg_breath_strength]
            avg_breath = sum(breath_vals) / len(breath_vals) if breath_vals else None
            last_played = max(s.started_at for s in level_sessions_completed)
        else:
            best_stars = 0
            avg_stars = 0.0
            avg_breath = None
        ...
        level_progress.append(LevelProgress(
            ...
            attempts=len(level_sessions_all),
    ...
    # Both calls below are blocking I/O -- get_latest_assessment does a sync
    # ORM query (see services/assessment_lookup.py); get_latest_decision
    # opens a sync SessionLocal per retraining/data_store.py's docstring.
    # Run via asyncio.to_thread so neither stalls this endpoint's event
    # loop, same pattern as _agent_service.detect_trend further down.
    latest_assessment = None
    if patient.assessment_patient_id:
        latest_assessment = await asyncio.to_thread(get_latest_assessment, str(patient.assessment_patient_id))
...
async def create_note(
    ...
    return NoteOut(
        ...
        session_id=None, content=note.content, tags=None,
...
async def list_notes(
    ...
    return [NoteOut(
        ...
        session_id=None, content=n.content, tags=None,
...
@router.post("/patients/{patient_id}/assignments", response_model=AssignmentOut, status_code=status.HTTP_201_CREATED)
...
@router.post("/patients/{patient_id}/messages", response_model=MessageOut, status_code=status.HTTP_201_CREATED)
...
async def mark_message_read(
    ...
    result = await db.execute(
        select(Message).join(BreathQuestPatient, Message.patient_id == BreathQuestPatient.id).where(
            Message.id == message_id, BreathQuestPatient.therapist_id == therapist.id,
        )
    )
...
@router.post("/patients/{patient_id}/home-practice", response_model=HomePracticeLogOut, status_code=status.HTTP_201_CREATED)
...
async def list_patient_alerts(
    ...
    played in `inactive_days`+ (across BreathQuest/VoiceHurdleRace/
    VaakMirror/Chime) or who have an overdue assignment."""
    patients_result = await db.execute(
        select(BreathQuestPatient).where(
            BreathQuestPatient.therapist_id == therapist.id,
            BreathQuestPatient.is_active == True,
        )
    )
    ...
    for p in patients:
        ...
        # detect_trend does synchronous DB I/O internally -- thread it off,
        # same fix as chime_last just above. Runs once per patient in this
        # loop, so on a route already O(n) in patient count, leaving it
        # blocking would compound per-patient.
        trend = await asyncio.to_thread(_agent_service.detect_trend, str(p.id))
...
async def get_weekly_summary(
    ...
    result = await generate_weekly_summary(db, patient, week_start, CHIME_DB_PATH)
...
async def get_sound_progress(
    ...
    sound-level correct/incorrect outcome with a timestamp."""
    await _get_owned_patient(patient_id, therapist, db)
    ...
    for sound_id, outcome, created_at in vm_result.all():
        ...
        if outcome_str in ("passed", "caught"):
            wk[0] += 1
    ...
    for sound_id, week_map in buckets.items():
        ...
        points.sort(key=lambda pt: pt.week_start)
...
async def list_home_practice_ideas(
    ...
    data, just filterable by condition/goal tag."""
    from app.services.home_practice_ideas import filter_ideas
...
async def get_patient_report(
    ...
    weekly_data = await generate_weekly_summary(db, patient, this_monday, CHIME_DB_PATH)
