 ...
 * Persists best scores per level in localStorage, per player
 */

const STORAGE_PREFIX = 'bq_scores_v1'
...
function getStorageKey(playerCode) {
  return playerCode ? `${STORAGE_PREFIX}_${playerCode}` : STORAGE_PREFIX
}

export function loadScores(playerCode = null) {
  try {
    return JSON.parse(localStorage.getItem(getStorageKey(playerCode)) || '{}')
  } catch { return {} }
}

export function saveScore(levelId, stars, playerCode = null, extraData = {}) {
  const scores = loadScores(playerCode)
  ...
  localStorage.setItem(getStorageKey(playerCode), JSON.stringify(scores))
