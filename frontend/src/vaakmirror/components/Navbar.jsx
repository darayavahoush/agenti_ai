...
export default function Navbar() {
  ...
  const links =
    auth?.kind === 'therapist'
      ? [
          { to: '/vaakmirror/patients', label: 'Patients' },
          { to: '/vaakmirror/dashboard', label: 'Dashboard' },
          { to: '/vaakmirror/exercises', label: 'Exercises' },
        ]
      : [{ to: '/vaakmirror', label: 'Games', end: true }]

  function handleLogout() {
    clearAuth()
    navigate('/vaakmirror/login')
  }

  return (
    <header className="sticky top-0 z-50 bg-ink/95 backdrop-blur border-b border-white/10">
      <nav className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <NavLink to="/vaakmirror" className="flex items-center gap-2 text-paper">
        ...
        <div className="flex items-center gap-3">
          ...
          {auth ? (
            ...
            location.pathname !== '/vaakmirror/login' && (
              <NavLink
                to="/vaakmirror/login"
