...
export default function Progress() {
  ...
  const patientSessions = sessions
    ...
    .sort((a, b) => new Date(a.created_at || 0) - new Date(b.created_at || 0)); // Sort old to new for progress line

  // Reverse sessions for display (newest first)
  const displaySessions = [...patientSessions].reverse();

  // Metrics for selected patient
  ...
  return (
    <div style={{ padding: "24px", background: "#f8fafc", minHeight: "calc(100vh - 80px)" }}>
      <h1 style={{ margin: "0 0 8px 0", color: "#4c1d95" }}>📈 Patient Progress Journey</h1>
      <p style={{ margin: "0 0 20px 0", color: "#64748b" }}>
        Review pronunciation improvement, timeline history, and clinical metrics for each child.
      </p>

      {loading ? (
        <p>Loading clinical progress data...</p>
      ) : patients.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "20px" }}>
            <div style={{ padding: "20px", background: "linear-gradient(135deg, #ede9fe 0%, #fff 100%)", borderRadius: "16px", boxShadow: "0 4px 12px rgba(0,0,0,0.05)", borderLeft: "6px solid #8b5cf6" }}>
              <h3 style={{ margin: "0 0 6px 0", color: "#6d28d9", fontSize: "14px", textTransform: "uppercase" }}>Average Accuracy</h3>
            ...
            <div style={{ padding: "20px", background: "linear-gradient(135deg, #f0fdf4 0%, #fff 100%)", borderRadius: "16px", boxShadow: "0 4px 12px rgba(0,0,0,0.05)", borderLeft: "6px solid #22c55e" }}>
              ...
              <p style={{ margin: "4px 0 0 0", fontSize: "13px", color: "#64748b" }}>
                Score: {bestSession.accuracy ?? 0}% match
              </p>
            </div>
