...
export default function BreathQuestPlay({ setPage }) {
  ...
  const handleRegister = () => {
    ...
    setTimeout(() => {
      const code = 'BQ' + Math.random().toString(36).substring(2, 5).toUpperCase()
      setRegistered({ player_code: code, first_name: firstName.trim() })
      setLoading(false)
    }, 1000)
  }
  ...
  if (registered) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center"
        ...
        <div className="text-7xl mb-4" style={{ animation: 'bounce 1s infinite' }}>{AVATAR_EMOJIS[avatar]}</div>
        <h1 className="text-4xl font-black text-white mb-2" style={{ fontFamily: 'sans-serif' }}>You're in! 🎉</h1>
        <p className="text-white/50 mb-8">Write this down so you can log back in:</p>
        <div className="bg-white/5 border-2 border-green-400 rounded-2xl p-8 mb-6 w-full max-w-xs">
          <p className="text-white/40 text-sm mb-1">Your Player Code</p>
          <p className="text-4xl font-black text-green-400 tracking-widest mb-4" style={{ fontFamily: 'sans-serif' }}>
            {registered.player_code}
          </p>
          <p className="text-white/40 text-sm mb-1">Your PIN</p>
          <p className="text-3xl font-bold text-amber-400 tracking-widest" style={{ fontFamily: 'sans-serif' }}>
            {'•'.repeat(pin.length)}
          </p>
        </div>
        <p className="text-white/30 text-xs mb-8">Show this to your therapist too!</p>
        <button 
          onClick={() => setPage('breathquest')}
          className="px-8 py-4 rounded-2xl font-bold text-xl bg-green-400 hover:bg-green-500 text-black transition-all"
        >
          Let's Play! 🚀
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6"
      ...
      <button 
        ...
        className="absolute top-6 left-6 text-white/30 hover:text-white/60 text-sm transition-colors"
      >
        ← Back
      </button>
      ...
      {mode === 'choose' && (
        <div className="text-center w-full max-w-sm">
          ...
          <h1 className="text-4xl font-black text-white mb-2" style={{ fontFamily: 'sans-serif' }}>BreathQuest</h1>
          <p className="text-white/40 mb-10">Ready to play?</p>
          <div className="flex flex-col gap-4">
            <button onClick={() => setMode('register')}
              className="p-6 rounded-2xl bg-gradient-to-br from-amber-400/20 to-orange-400/20
                         border-2 border-amber-400/40 hover:border-amber-400
                         transition-all hover:scale-105 text-left">
              <div className="text-3xl mb-2">✨</div>
              <p className="text-xl font-bold text-white" style={{ fontFamily: 'sans-serif' }}>New Player</p>
              <p className="text-white/40 text-sm">Create your account</p>
            </button>
            <button onClick={() => setMode('login')}
              className="p-6 rounded-2xl bg-gradient-to-br from-green-400/20 to-teal-400/20
                         border-2 border-green-400/40 hover:border-green-400
                         transition-all hover:scale-105 text-left">
              <div className="text-3xl mb-2">🔑</div>
              <p className="text-xl font-bold text-white" style={{ fontFamily: 'sans-serif' }}>I have a code</p>
      ...
      {mode === 'register' && (
        <div className="w-full max-w-sm">
          ...
          <h1 className="text-3xl font-black text-white mb-6 text-center" style={{ fontFamily: 'sans-serif' }}>Create Account</h1>

          {/* Name */}
          <div className="mb-4">
            <label className="text-sm text-white/50 block mb-1">Your first name</label>
            <input className="w-full p-3 rounded-xl bg-white/10 border border-white/20 text-white text-lg focus:outline-none focus:border-green-400"
            ...
            {AVATARS.map(av => (
              <button key={av} onClick={() => setAvatar(av)}
                className={`h-12 rounded-xl text-2xl transition-all border-2
                  ${avatar === av ? 'border-green-400 bg-green-400/20 scale-110' : 'border-white/10 bg-white/5'}`}>
                {AVATAR_EMOJIS[av]}
              </button>
            ))}
          </div>
          ...
          <div className="flex justify-center gap-3 mb-4">
            {[0,1,2,3].map(i => (
              <div key={i} className={`w-5 h-5 rounded-full border-2 transition-all
                ${pin.length > i ? 'bg-green-400 border-green-400' : 'border-white/30'}`} />
            ))}
          </div>
              ...
              <button key={i}
                ...
                className={`h-14 rounded-xl text-xl font-bold transition-all active:scale-95
                  ${d==='' ? 'invisible' : d==='⌫' ? 'bg-white/5 text-white/50 hover:bg-white/10'
                    : 'bg-white/10 text-white hover:bg-green-400/20 hover:text-green-400'}`}
          ...
          {error && <p className="text-orange-400 text-sm text-center mb-3">{error}</p>}
          <button className="w-full px-8 py-4 rounded-2xl font-bold text-xl bg-green-400 hover:bg-green-500 text-black transition-all" 
      ...
      {mode === 'login' && (
        <div className="w-full max-w-sm">
          <button onClick={() => { setMode('choose'); setPin(''); setError('') }}
                  className="text-white/30 hover:text-white/60 text-sm mb-6 transition-colors">← Back</button>
          <h1 className="text-3xl font-black text-white mb-6 text-center" style={{ fontFamily: 'sans-serif' }}>Welcome Back!</h1>

          <div className="mb-4">
            <label className="text-sm text-white/50 block mb-1">Your Player Code</label>
            <input className="w-full p-3 rounded-xl bg-white/10 border border-white/20 text-white text-center text-xl font-bold tracking-widest uppercase focus:outline-none focus:border-green-400"
